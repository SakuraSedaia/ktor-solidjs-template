...
interface AsyncFetchProps {
  /**
   * Route relative to the Ktor API's `/api/v1/` base path.
   *
   * @example `users/1`
   */
  apiRoute: string,
 ...
 * helper sends requests to the Ktor API Wrapper. During frontend development,
 * Vite proxies that path to the Ktor server; in production, Ktor serves both
 * the API and the built SolidJS application from the same origin.
 *
 * @remarks
...
export async function asyncFetch<DataType>({apiRoute, errorMessage = "Failed to fetch data",}: AsyncFetchProps): Promise<DataType> {
  const res = await fetch(`${apiRoot}/${apiRoute}`)
